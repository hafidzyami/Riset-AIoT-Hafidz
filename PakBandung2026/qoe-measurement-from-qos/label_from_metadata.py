...
AMBANG_STALL = 1.0      # detik; parameter operasional, BUKAN ketentuan ITU-T


def classify(mean_o22, stall_s, ambang=None):
    """Aturan pelabelan: stalling dominan; jika mulus, pakai O22 rata-rata window.

    Batas skor O22 diselaraskan pada anchor kategori ACR ITU-T P.910. Ambang
    stalling adalah parameter yang ditetapkan penelitian ini, sehingga
    sensitivitasnya perlu diuji dan dilaporkan.
    """
    if stall_s > (AMBANG_STALL if ambang is None else ambang):
        return "Critical"
    if stall_s > 0.0:
        return "Degraded"
    if mean_o22 >= 4.0:
...
def main():
    ...
    ap.add_argument("--stall-threshold", type=float, default=1.0,
                    help="ambang stall (detik) utk kelas Critical; default 1.0. "
                         "Coba 0.5 dan 2.0 untuk analisis sensitivitas")
    ...
    global AMBANG_STALL
    AMBANG_STALL = a.stall_threshold
